...
static if (ZMQ_VERSION_MAJOR == 2) {
	...
	{
		//#   if defined (ZMQ_LINGER) -- todo how to port it to D?
		static if (ZMQ_LINGER) { 
		    int rc = zmq_setsockopt (zocket, ZMQ_LINGER, &linger, int.sizeof);
		    assert (rc == 0 || errno == ETERM);
		}
	}
	...
	void
	zsocket_set_subscribe (void *zocket, string subscribe)
	{
		//#   if defined (ZMQ_SUBSCRIBE) -- todo how to port it to D?
		static if(ZMQ_SUBSCRIBE) {
		    if (zsockopt_type (zocket) != ZMQ_SUB) {
		        writef ("ZMQ_SUBSCRIBE is not valid on %s sockets\n", zsocket_type_str (zocket));
		        assert (false);
		    }
		    int rc = zmq_setsockopt (zocket, ZMQ_SUBSCRIBE, subscribe.ptr, subscribe.length);
		    assert (rc == 0 || errno == ETERM);
		}
	}
	...
	{
		//#   if defined (ZMQ_TYPE) -- todo how to port it to D?
		static if(ZMQ_TYPE){
		    int type;
		    size_t option_len = int.sizeof;
	...
	{
		//#   if defined (ZMQ_RCVMORE) -- todo how to port it to D?
		static if (ZMQ_RCVMORE) {
		    int64_t rcvmore;
		    size_t option_len = int64_t.sizeof;
...
static if (ZMQ_VERSION_MAJOR == 3) {
	...
	{
		//#   if defined (ZMQ_SNDHWM) -- todo how to port it to D?
		static if (ZMQ_SNDHWM) {
		    int rc = zmq_setsockopt (zocket, ZMQ_SNDHWM, &sndhwm, int.sizeof);
		    assert (rc == 0 || errno == ETERM);
		}
	}
	...
	{
		//#   if defined (ZMQ_RCVHWM) -- todo how to port it to D?
		static if (ZMQ_RCVHWM) { 
		    int rc = zmq_setsockopt (zocket, ZMQ_RCVHWM, &rcvhwm, int.sizeof);
		    assert (rc == 0 || errno == ETERM);
		}
	}
	...
	zsocket_set_subscribe (void *zocket, string subscribe)
	{
		...
		static if(ZMQ_SUBSCRIBE) {
		    if (zsocket_type (zocket) != ZMQ_SUB) {
		        writefln ("ZMQ_SUBSCRIBE is not valid on %s sockets", zsocket_type_str (zocket));
		        assert (false);
		    }
		    int rc = zmq_setsockopt (zocket, ZMQ_SUBSCRIBE, subscribe.ptr, subscribe.length);
		    assert (rc == 0 || errno == ETERM);

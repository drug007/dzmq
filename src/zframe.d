...
{
    auto self = new zframe_t;
    if (!self)
        return null;

    if (size) {
        zmq_msg_init_size (&self.zmsg, size);
        zmq_msg_data (&self.zmsg)[0..size] = data[0..size]; 
    }
    else
...
zframe_t *
zframe_new (ref string data)
{
	return zframe_new(toStringz(data), data.length);
}
...
//
//void
//zframe_reset (zframe_t *self, string data){
//	zframe_reset(self, toStringz(data), data.length);
//}
//
//////  --------------------------------------------------------------------------
//////  Selftest
